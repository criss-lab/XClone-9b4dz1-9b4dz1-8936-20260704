...
export async function pollFediverseInbox(userId: string): Promise<any[]> {
  try {
    ...
    // Fallback: local activitypub_inbox table (uses already-imported supabase client)
    try {
...
// ── Gateway Relay (via Supabase Edge Function) ────────────────────────────────

/**
 * gwRelay — calls the gateway via our server-side relay edge function.
 * Use this instead of direct gateway calls when you need to avoid CORS
 * or keep auth tokens server-side.
 */
